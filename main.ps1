...
function Main {
    $file = Read-Host -Prompt "Absolute path to excel file"
    ...
    foreach ($sheet in $workbook.WorkSheets) {
        ...
        for ($row = $startRow; $row -le $rowEnd; $row++) {
            ...
            for ($column = $startColumn; $column -le $columnEnd; $column++) {
                ...
                $columns += @{$columnHead = $value } # Add all values in row to $columns
            }
    ...
    [PSCustomObject]$myCustomObject | ConvertTo-Json -Depth 3 | Set-Content -Path "$($path)\psobject.json"
}
